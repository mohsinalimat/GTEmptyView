...
- (void)setGt_emptyView:(GTEmptyView *)gt_emptyView
{
    if (gt_emptyView != self.gt_emptyView) {
        ...
        for (UIView *view in self.subviews) {
            if ([view isKindOfClass:[GTEmptyView class]]) {
...
- (GTEmptyView *)gt_emptyView {
    return objc_getAssociatedObject(self, _cmd);
}

- (void)setGt_loadingView:(GTLoadingView *)gt_loadingView
{
    if (gt_loadingView != self.gt_loadingView) {
        ...
        for (UIView *view in self.subviews) {
            if ([view isKindOfClass:[GTLoadingView class]]) {
                [view removeFromSuperview];
            }
        }
