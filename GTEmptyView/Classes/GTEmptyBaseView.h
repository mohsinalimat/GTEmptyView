...
/**
 事件回调
 */
typedef void (^GTActionTapBlock)(void);

@interface GTEmptyBaseView : UIView
...
@property (nonatomic, copy) GTActionTapBlock tapContentViewBlock;

/////////属性传递 (这些属性只用来传递，修改无效)
@property (nonatomic, weak, readonly)   id actionBtnTarget;
@property (nonatomic,assign,readonly)   SEL actionBtnAction;
@property (nonatomic, copy, readonly)   GTActionTapBlock btnClickBlock;
...
+ (instancetype)emptyActionViewWithImageStr:(NSString *)imageStr
                                  ...
                                  detailStr:(NSString *)detailStr
                                btnTitleStr:(NSString *)btnTitleStr
                              btnClickBlock:(GTActionTapBlock)btnClickBlock;
